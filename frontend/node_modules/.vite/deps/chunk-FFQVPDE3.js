...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-2BZVL2SU.js");
    ...
    if (options.type === "erc721") {
      if (options.singlePhaseDrop) {
        const { claimCondition } = await import("./claimCondition-SVB6QVKO.js");
        return await claimCondition({
          contract: options.contract
        });
      }
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-UN22KYYR.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    ...
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-GHWYUACP.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-644RFLVM.js");
...
//# sourceMappingURL=chunk-FFQVPDE3.js.map
